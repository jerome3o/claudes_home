...
// Server Setup
// ============================
const server = new Server({ name: 'claude-web-mcp', version: '1.0.0' }, { capabilities: { tools: {} } });
// List tools handler
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        switch (name) {
            ...
            case 'create_session': {
                const { name: sessionName, folder } = args;
            ...
            case 'send_message': {
                const { session_id, content, sender_session_id, sender_session_name, } = args;
            ...
            case 'get_session_history': {
                ...
                const endpoint = format === 'messages'
            ...
            case 'subscribe_webhook': {
                const { session_id, webhook_path } = args;
                const result = await apiCall('POST', `/api/sessions/${session_id}/subscribe`, { webhook_path });
                return {
                    content: [
                        {
                            type: 'text',
            ...
            case 'unsubscribe_webhook': {
                const { session_id, webhook_path } = args;
                const result = await apiCall('DELETE', `/api/sessions/${session_id}/subscribe/${webhook_path}`);
                return {
                    content: [{ type: 'text', text: JSON.stringify(result, null, 2) }],
                };
            }
            case 'list_webhook_subscriptions': {
                ...
                const subs = await apiCall('GET', `/api/sessions/${session_id}/subscriptions`);
                return {
            ...
            case 'hub_list_posts': {
                const { topic_id, limit, offset } = args;
            ...
            case 'hub_subscribe': {
                const { session_id, subscription_type, target_id } = args;
            ...
            case 'hub_upload_file': {
                const { filename, content_base64, subfolder } = args;
